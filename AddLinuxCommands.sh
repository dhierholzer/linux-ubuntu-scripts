...
EOF
sudo mv -f update /usr/local/bin/
...
EOF3
sudo mv -f restart /usr/local/bin/
sudo chmod +x /usr/local/bin/restart
...
sudo systemctl reload $RELOAD
echo "Service Reloaded"
EOF4
sudo mv -f reload /usr/local/bin/
sudo chmod +x /usr/local/bin/reload
...
EOF5
sudo mv -f services /usr/local/bin/
sudo chmod +x /usr/local/bin/services
...
echo "End Of Transmission"
EOF6
sudo mv -f services-all /usr/local/bin/
sudo chmod +x /usr/local/bin/services-all
...
echo "End Of Transmission"
EOF7
sudo mv -f allservices /usr/local/bin/
sudo chmod +x /usr/local/bin/allservices
